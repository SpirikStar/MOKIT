...
18. Создание пользователя(Регистрация)

18.1. Необходимо проверить что логин уникальный. 

> `get` - метод, который ищет данные в модели. Важно: метод может вернуть только один элемент. Если в результате метод ничего не найдёт, вернётся ошибка.
...
18.2. Создание пользователя в моделе User(БД). 
> `create_user` - метод, который создёт пользователя и шифрует пароль
